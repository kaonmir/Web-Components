  ...
  render() {
    ...
    if (orientation === "tiptoe") {
      hexagon_row = {
        display: "flex",
        width: `${
          (Math.sqrt(3) * length) / 2 +
          margin +
          col * (Math.sqrt(3) * length + margin) -
          margin / 2
        }px`,
      };
      ...
      box = {
        paddingBottom: `${length / 2 - (3 / 2 / Math.sqrt(3)) * margin}px`,
      };
    } else {
      hexagon_col = {
        display: "inline-flex",
        ...
        height: `${
          (Math.sqrt(3) * length + margin) * row + (Math.sqrt(3) * length) / 2
        }px`,
    ...
    return <div style={box}>{orientation === "tiptoe" ? rows : cols}</div>;
  }
}
